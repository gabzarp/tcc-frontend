    ...
    <div class="d-flex bg-white border border-muted rounded-lg p-2 h-100">
      ...
      <div class="col-8 text-dark pt-2">
        ...
        <p class="mb-1">{{memberData.position}}</p>
          <div v-if="isScrumMaster && !isNaN(dueDate)">Prazo: {{dueDate}}</div>
          <div v-if="isScrumMaster && !isNaN(communication)">Comunicação: {{communication}}</div>
          <div v-if="isScrumMaster && !isNaN(technicalKnowledge)">Conhecimento técnico: {{technicalKnowledge}}</div>
